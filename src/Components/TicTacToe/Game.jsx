...
const Game = () => {
    ...
    const toggle = (e,num) =>{
        ...
        if(count%2===0)
        {
            e.target.innerHTML =`<img src='${icon1}'>`;
            data [num]="x";
            setCount(++count);
        }
        else{
            e.target.innerHTML =`<img src='${icon2}'>`;
            data [num]="o";
            setCount(++count);
    ...
    const won =(winner)=>{
        setLock(true);
        if(winner==="x"){
            titleref.current.innerHTML=`Congractulations : <img src='${icon1}'>`
        }
        else {titleref.current.innerHTML=`Congractulations : <img src='${icon2}'>`}
    }
    const reset =()=>{
        setLock(false);
        data = ["","","","","","","","",""];
        titleref.current.innerHTML='Tic Tac Toe Game For  <span> : You </span>'
        box_array.map((e)=>{
            e.current.innerHTML ="";
        })
    }
  return (
    <div className='container'>
      <h1 className="title" ref={titleref}>Tic Tac Toe Game For   <span>: You</span></h1>
